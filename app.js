import { getRandomNumber, getCssProp, l, detectColision, roundNum } from '/utils.js';
...
function handleCharacterCollisions() {
    ...
    if ( colisionBlock && !colisionHole ) {
        ...
        soundCount++
        if ( soundCount > 35 ) {
            (new Audio('/music/birdsoundshole.wav')).play()
            soundCount = 0
        }
...
function gameOver() {
    (new Audio('/music/gameover.wav')).play()
    gameStopped = true;
    showGameoverscreen();
    stopBlockAnimation();
    stopGravity();
